...
function validateJWTToken(token) {
    ...
    try {
        parts.forEach(part => {
            ...
            (window.atob || atob)(part.replace(/-/g, '+').replace(/_/g, '/'));
        });
        return true;
    }
    catch {
...
class AudioManager {
    ...
    async initialize() {
        try {
            // Request microphone access with proper binding
    ...
    async loadWorkletProcessors() {
        ...
        const pcmProcessorUrl = (window.URL || window.webkitURL).createObjectURL(pcmBlob);
        await this.audioContext.audioWorklet.addModule(pcmProcessorUrl);
        (window.URL || window.webkitURL).revokeObjectURL(pcmProcessorUrl);
        ...
        const ttsProcessorUrl = (window.URL || window.webkitURL).createObjectURL(ttsBlob);
        await this.ttsAudioContext.audioWorklet.addModule(ttsProcessorUrl);
        (window.URL || window.webkitURL).revokeObjectURL(ttsProcessorUrl);
    }
    getPCMProcessorCode() {
        return `
    ...
    base64ToInt16Array(base64) {
        const raw = (window.atob || atob)(base64);
