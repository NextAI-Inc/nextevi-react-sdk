...
(function (ErrorCode) {
    ...
    ErrorCode["JWT_TOKEN_EXPIRED"] = "JWT_TOKEN_EXPIRED";
    ErrorCode["JWT_TOKEN_INVALID"] = "JWT_TOKEN_INVALID";
    ErrorCode["PROJECT_NOT_FOUND"] = "PROJECT_NOT_FOUND";
    ErrorCode["CONFIG_NOT_FOUND"] = "CONFIG_NOT_FOUND";
    ErrorCode["IDLE_TIMEOUT_WARNING"] = "IDLE_TIMEOUT_WARNING";
    ErrorCode["IDLE_TIMEOUT_HANGUP"] = "IDLE_TIMEOUT_HANGUP";
    ErrorCode["TURN_DETECTION_FAILED"] = "TURN_DETECTION_FAILED";
})(ErrorCode || (ErrorCode = {}));
 ...
 */
function createBoundBrowserAPIs() {
...
const BrowserAPIs = createBoundBrowserAPIs();
// Export individual APIs for convenience
const { getUserMedia, atob: boundAtob, btoa: boundBtoa, createObjectURL, revokeObjectURL, createAudioContext, getLocationInfo, isEnvironmentSupported: isEnvironmentSupported$1, getUserAgent } = BrowserAPIs;
/**
...
function checkAPISupport() {
    const support = {
        ...
        url: typeof URL !== 'undefined' || typeof window.webkitURL !== 'undefined',
 ...
 */
function isJWTAuth(config) {
    return 'accessToken' in config || config.type === 'jwt';
}
/**
 * Check if configuration uses API key authentication
 */
function isAPIKeyAuth(config) {
    return 'apiKey' in config || config.type === 'apiKey' || !('type' in config) || config.type === undefined;
}
/**
 ...
 */
function createJWTConnectionConfig(accessToken, configId, options = {}) {
    const config = {
        type: 'jwt',
 ...
 */
function createUnifiedConnectionConfig(auth) {
...
class WebSocketManager {
    ...
    async connect(config) {
        ...
        try {
            ...
            if (headers && Object.keys(headers).length > 0) {
                ...
                this.websocket._nextevi_headers = headers;
            }
            else {
                this.websocket = new WebSocket(wsUrl);
            }
    ...
    buildWebSocketUrlWithAuth(config) {
        const baseUrl = config.websocketUrl || this.getDefaultWebSocketUrl();
        const params = new URLSearchParams();
        let headers = {};
        if (isJWTAuth(config)) {
    ...
    handleTurnCompleteMessage(message) {
        const turnResult = {
    ...
    handleIdleWarningMessage(message) {
        const warning = {
            type: 'idle_warning',
...
class AudioManager {
     ...
     */
    async ensureAudioContextsRunning() {
        let success = true;
    ...
    async initialize() {
        try {
            ...
            if (permissionStatus === 'denied') {
                throw new NextEVIError('Microphone access denied. Please enable microphone permissions.', ErrorCode.MICROPHONE_ACCESS_DENIED);
            }
...
function VoiceProvider({ children, debug = false }) {
    ...
    return (jsx(VoiceContext.Provider, { value: contextValue, children: children }));
}
...
function useSimpleVoice(options = {}) {
    ...
    const connectSimple = useCallback(async (apiKey, projectId, configId, websocketUrl) => {
 ...
 */
function useVoiceTurnDetection() {
    ...
    const turnCount = messages.filter(msg => msg.type === 'user' && msg.metadata?.isFinal).length;
 ...
 * ```
 */
function quickStartJWT(config) {
    const connectionConfig = createJWTConnectionConfig(config.accessToken, config.configId, {
 ...
 * ```
 */
function quickStartUnified(config) {
...
export { CONNECTION_STATES, DEFAULT_CONFIG, NEXTEVI_VERSION, VoiceProvider, configFromEnvironment, createConnectionConfig, createJWTConnectionConfig, createUnifiedConnectionConfig, devUtils, generateConnectionId, getBrowserInfo, getDefaultWebSocketUrl, isAPIKeyAuth, isAudioSupported, isDevelopment, isEnvironmentSupported, isJWTAuth, maskApiKey, quickStart, quickStartJWT, quickStartUnified, requestMicrophonePermission, useSimpleVoice, useVoice, useVoiceAudio, useVoiceDebug, useVoiceIdleTimeout, useVoiceMessages, useVoiceStatus, useVoiceTurnDetection, validateApiKey, validateConfig, validateConfigId, validateJWTToken, validateProjectId };
